...
[stem]
++++
\hat{X} = (\hat{x}_0,\hat{x}_1,\hat{x}_2)^T : \mbox{point dans} \hat{K}\\
\hat{X}_i = (\hat{x}_{i,0},\hat{x}_{i,1},\hat{x}_{i,2})^T: \mbox{sommets de } \hat{K}\\
X_i = (x_{i,0},x_{i,1},x_{i,2})^T = T_K(\hat{X}_i), \quad i=1 \ldots d \mbox{sommets de K}\\
\psi_i \mbox{ ieme fonction de base de Lagrange sur } \hat{K} \mbox{ associée à } \hat{x}_i\\
x_j = T_K(\hat{x})_j = \sum_{i=1}^3 x_{i,j} \psi_i(\hat{x}), \quad j=1\ldots 2\\
\mbox{matrice jacobienne } \hat{\nabla} T_K(\hat{x}) = (\hat{\nabla} T_K (\hat{x})_0, \hat{\nabla} T_K (\hat{x})_1, \hat{\nabla} T_K (\hat{x})_2 ) = \mbox{CONSTANTE indep de } \hat{x}\\
\mbox{calcul } |det \nabla T_K |\\
\mbox{calcul } \nabla T_K ^{-1}\\
++++
