...
Definition::
In the previous table, we call *multiplicity* the number of permutations that must be performed on the barycentric coordinates to obtain the list of all the points of the quadrature. 

Example::
The first-order formula has 3 points; the corresponding barycentric coordinates and weights are
[stem]
++++
